...
6. For schema changes, note that Liquibase is env-gated: `spring.liquibase.enabled` defaults to `false` (`SPRING_LIQUIBASE_ENABLED`), on by default only in the `local` profile (`seed` context) and off in `testing`.
7. For auth-sensitive changes, inspect `WebSecurityConfig` (`/tenant/public/**` is permitAll), authority mapping, and tenant resolver classes before editing controller rules.
